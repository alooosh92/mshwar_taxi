...
import '../../models/add_trip.dart';
import '../../models/add_user_location.dart';
import '../../models/get_driver_endless_trip.dart';
import '../../models/get_user_endless_trip.dart';
import '../../models/send_driver_state.dart';
import '../../models/show_trip.dart';
import '../../models/trip_for_driver.dart';
import '../../models/trip_model_for_socket.dart';
...
import 'widget/buttom_sheet.dart';
...
class TripController extends GetxController {
  ...
  Future<List<TripForDriver>> getDriverTrips() async {
    http.Response response = await http.get(HosttingTaxi.getDriverTrip,
        headers: HosttingTaxi().getHeader());
    if (response.statusCode == 200) {
      try {
        var body = jsonDecode(response.body);
        List<TripForDriver> list = [];
        for (var element in body) {
          var trip = TripForDriver.fromJson(element);
          list.add(trip);
        }
        return list;
      } catch (e) {
    ...
    if (response.statusCode == 200) {
      ...
      if (b.toString().contains('The Balance not Enough')) {
        Get.dialog(AlertDialog(
          ...
          content: const Text(
              'لا تملك رصيد كافي لقبول هذه الرحلة الرجاء اعادة شحن رصيدك.'),
        ));
      } else {
        if (response.statusCode == 500 &&
          ...
          Get.dialog(
            AlertDialog(
              ...
              content:
                  const Text('تمت الموافقة على هذه الرحلة من قبل سائق اخر'),
            ),
          );
        }
      }
  ...
  void deleteTripNow(int id) {
    http.delete(HosttingTaxi.deleteTrip(id),
        headers: HosttingTaxi().getHeader());
  }
    ...
    if (body['event'] == "App\\Events\\TripDeleteEvent" ||
      ...
      if (storeg.read('role') == 'driver') {
        ...
        if (storeg.read('role') == 'user') {
          if (body['event'] == "App\\Events\\ChangeStatusDriverEvent") {
            var car = SendDriverStateModel.fromJson(dat['data']);
            BitmapDescriptor icon = await getIcon(car);
            mark.add(Marker(
  ...
  Future<BitmapDescriptor> getIcon(SendDriverStateModel car) async {
    if (car.companyName == null) {
      return getIconCompanyNull(car);
    } else {
      ...
      if (getUserEndLessTrip != null &&
        ...
        img = '${dir.path}/${car.companyName}_bas.png';

        stat = 'bas';
      } else {
        if (car.state == 'busy') {
          img = '${dir.path}/${car.companyName}_busy.png';
          stat = 'busy';
        } else {
          img = '${dir.path}/${car.companyName}_available.png';
          stat = 'available';
        }
      }
      var file = File(img);
      if (file.existsSync() && file.lengthSync() > 0) {
        return BitmapDescriptor.fromBytes(file.readAsBytesSync());
      } else {
        http.Response response =
            await http.get(HosttingTaxi.image(car.companyName!, stat));
        if (response.statusCode == 200) {
          File(img).writeAsBytes(response.bodyBytes);
  ...
  Future<BitmapDescriptor> getIconCompanyNull(SendDriverStateModel car) async {
